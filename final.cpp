		...
		void run(){
			...
			do{
				...
				if(now.get()>=ring_time.get()){
					...
					while(choise==2){
						if(choise==2) choise2();
						cout<<RED_BOLD"貪睡時間已過！！！"RESET<<endl;
						cout<<Blink"選1：起床，選2：繼續睡"RESET<<endl;
						system("play cc.mp3");
						cin>>choise;
						if(choise==1) choise1();
...
int main(){
	...
	cout <<"\033[40;35m請設定鬧鐘時間：(?時?分?秒)"<<endl<<"一次輸入一行"<<RESET<<endl;
	cin >> h1 >> m1 >> s1;
	...
	num_min=0;
	count=0;
	char t1ime_now[10]={},t1ime_hour[100]={},t1ime_min[10]={},t1ime_sec[10]={},t1mp[100]={},t1mp1[100]={},t1mp2[10]={},t1mp3[10];
	time ( &rawtime );
	timeinfo = localtime ( &rawtime );
	sscanf(asctime (timeinfo),"%s %s %s %s",t1mp,t1mp1,t1mp2,t1ime_now);//分段
	sscanf(t1ime_now,"%[^:]",t1ime_hour);//取小時

	for(int i=0;i<8;i++){
		if(t1ime_now[i]==':') count++;
		if(t1ime_now[i]!=':' && count == 1){
			t1ime_min[num_min++] = t1ime_now[i];
		}
		if(t1ime_now[i]!=':' && count == 2){
			t1ime_sec[num_sec++] = t1ime_now[i];
		}
	}//時間輸入
srand(time(NULL));


	c.adjust_now(atoi(t1ime_hour),atoi(t1ime_min),atoi(t1ime_sec));     //起始時間
...
void choise1(){
	...
	if(x<=2&& strcmp(ans,correct_answer[x])==0){  //腦經急轉彎答對者
		cout<<"\033[40;34;5m 恭喜你答對了!"RESET<<endl;
	}
...
void choise2(){
	for(int i=0;i<10;i++){
		tick1();
		cout<<"\033[32;5m趕快起床,再賴床你試試看!"<<'\a'<<'\a'<<'\a'<<RESET<<endl;
	}
